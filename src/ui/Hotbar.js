  ...
  render(ctx) {
    ...
    for (let i = 0; i < this.slotCount; i++) {
      ...
      if (slotTexture && selectedTexture) {
        ctx.drawImage(isSelected ? selectedTexture : slotTexture, x, y, this.slotSize, this.slotSize);
      } else {
        // Fallback drawing
        ctx.fillStyle = isSelected ? 'rgba(100, 100, 100, 0.9)' : 'rgba(50, 50, 50, 0.8)';
      ...
      // Draw slot number (with shadow)
      ctx.font = '10px monospace';
      ctx.textAlign = 'left';
      ctx.textBaseline = 'top';
      
      ctx.fillStyle = '#000000';
      ctx.fillText((i + 1) % 10, x + 3, y + 3); // Shadow
