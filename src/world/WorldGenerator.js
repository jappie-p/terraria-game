...
export class WorldGenerator {
  ...
  generateVegetation(chunk) {
    ...
    for (let localX = 2; localX < CHUNK_SIZE - 2; localX++) {
      ...
      // Tree density noise - use consistent seed
      const treeValue = this.noise.noise2D01(worldX * 0.08, 12345);

      // Check if we should plant a tree here (roughly every ~12 blocks)
      if (treeValue > 0.82) {
        // Find surface Y at this X within chunk bounds
        let surfaceLocalY = -1;
        for (let localY = 0; localY < CHUNK_SIZE; localY++) {
        ...
        if (surfaceLocalY !== -1 && surfaceLocalY > 6) { // Need space above for tree
          this.plantTree(chunk, localX, surfaceLocalY);
          // Skip a few blocks so trees don't overlap
          localX += 4;
        }
      }
    }
  ...
  // Plant a tree at position (localX, groundLocalY are chunk-local coordinates)
  plantTree(chunk, localX, groundLocalY) {
    // Tree height (seeded by position for consistency)
    const worldX = chunk.x * CHUNK_SIZE + localX;
    const heightNoise = this.noise.noise2D01(worldX * 0.5, 9999);
    const height = 4 + Math.floor(heightNoise * 4); // 4-7 blocks high

    // Trunk
    for (let i = 1; i <= height; i++) {
      const y = groundLocalY - i;
      if (y >= 0 && y < CHUNK_SIZE) {
        chunk.setTile(localX, y, Tile.create(TILE.LOG));
      }
    }

    // Leaves - create a nice canopy
    const topY = groundLocalY - height;
    const leafRadius = 2;
    ...
    for (let dy = -1; dy <= 2; dy++) {
      for (let dx = -leafRadius; dx <= leafRadius; dx++) {
        ...
        // Shape the leaves (diamond pattern)
        const dist = Math.abs(dx) + Math.abs(dy);
        if (dist <= leafRadius + 1) {
          const currentTile = chunk.getTile(x, y);
          if (!currentTile || currentTile.type === TILE.AIR) {
            chunk.setTile(x, y, Tile.create(TILE.LEAVES));
          }
        }
      }
    }
