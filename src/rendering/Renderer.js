...
export class Renderer {
  ...
  render(world, entities, uiElements = [], input = null) {
    this.clear();

    if (!this.camera) return;

    // Store input for rendering methods that need it
    this.input = input;
    ...
    if (player) {
      ...
      this.renderPlacementPreview(player, world);
    }
  ...
  // Render chest interaction prompt ("F" key)
  renderChestPrompt(chestPos) {
    if (!chestPos) return;

    const worldX = chestPos.x * TILE_SIZE + TILE_SIZE / 2;
    const worldY = chestPos.y * TILE_SIZE - 10;
    const screenPos = this.camera.worldToScreen({ x: worldX, y: worldY });
    ...
    this.ctx.fillText('F', screenPos.x, screenPos.y);

    this.ctx.restore();
  }

  // Render block placement preview
  renderPlacementPreview(player, world) {
    // Only show preview if player has a placeable item selected
    const selectedSlot = player.inventory.getSlot(player.selectedSlot);
    if (!selectedSlot || selectedSlot.item.type !== 'BLOCK') return;
    if (selectedSlot.item.tileType === null || selectedSlot.item.tileType === undefined) return;

    // Get target tile position
    if (!this.input || !this.input.mouse) return;

    const worldPos = this.camera.screenToWorld(this.input.mouse.pos);
    const tileX = Math.floor(worldPos.x / TILE_SIZE);
    const tileY = Math.floor(worldPos.y / TILE_SIZE);

    // Check if in reach
    const playerTileX = Math.floor(player.pos.x / TILE_SIZE);
    const playerTileY = Math.floor(player.pos.y / TILE_SIZE);
    const dx = tileX - playerTileX;
    const dy = tileY - playerTileY;
    const distance = Math.sqrt(dx * dx + dy * dy);

    if (distance > player.reach) return;

    // Check if tile is empty
    const tile = world.getTile(tileX, tileY);
    const canPlace = tile && tile.type === 0;
    ...
    const playerInWay = !(playerBounds.right < tileBounds.left ||
    ...
    const screenPos = this.camera.worldToScreen({ x: screenX, y: screenY });

    this.ctx.save();

    if (canPlace && !playerInWay) {
